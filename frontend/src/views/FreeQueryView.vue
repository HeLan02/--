        ...
        <el-table :data="rows" style="width: 100%" v-loading="loading">
          ...
          <el-table-column label="设备">
            <template #default="{ row }">
              <el-tag v-if="row.hasMultimedia" type="success" style="margin-right: 6px">多媒体</el-tag>
              <el-tag v-if="row.hasAc" type="info">空调</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="120">
            <template #default="{ row }">
              <el-button size="small" type="primary" @click="toApply(row)">立即预约</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
...
function toApply(row) {
  router.push({
    ...
    query: {
      classroomId: row.id,
      buildingId: buildingId.value ?? row.buildingId,
      date: date.value,
      startPeriod: startPeriod.value,
      endPeriod: endPeriod.value,
    },
